...
public class App_4 {
	static void test(){
		...
		Map<String,String> params = Stream.of(queryString.split("&")).map(str->str.split("=")).collect(
	...
	static void test2(){
		...
		//*******************************************************************************************************************
		{
			books().stream().sorted((book1, book2) -> Double.compare(book1.getPrice(), book2.getPrice()))
		...
		{
			Comparator<Book> compa = (book1, book2) -> Double.compare(book1.getPrice(), book2.getPrice());
			books().stream().sorted(compa
					.thenComparing((book1, book2) -> book1.getPublishDate().isAfter(book2.getPublishDate()) ? -1 : 1))
					.forEach(System.out::println);
		...
		//**********************************************************************************************************************
		{
			...
			book = books().stream()
					.collect(Collectors.maxBy(comp.thenComparing(Comparator.comparing(Book::getPublishDate))));
			System.out.println(book);
		}
		...
		//**********************************************************************************************************************
		{
			...
			Map<String, Double> booksSum = books().stream()
...

		{
			...
			Map<String, Optional<Book>> booksMaxPrice = books().stream()
					.collect(Collectors
							.groupingBy(Book::getType, Collectors.maxBy(Comparator.comparing(Book::getPrice))));
			System.out.println(booksMaxPrice);

			Map<String, Optional<Book>> booksMinPrice = books().stream()
					.collect(Collectors
							.groupingBy(Book::getType, Collectors.minBy(Comparator.comparing(Book::getPrice))));
			System.out.println(booksMinPrice);

			Map<String, Optional<Book>> booksMaxPubDate = books().stream().collect(
		...
		//**********************************************************************************************************************
		{
			books().stream().filter(book -> book.getPrice() >= 80)
